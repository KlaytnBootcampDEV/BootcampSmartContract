...
contract Fastfood{
    ...
    function getIngredient(string memory nameIngredient) public view returns(uint256) {
        uint256 calo = ingredient[nameIngredient];
        return calo;
    }

    function addIngredient(string memory name, uint256 calo) virtual public {
        ingredient[name] = calo;
        emit AddIngredient(name, calo);
    }

    function addIngredients(string[] memory _name, uint256[] memory _caloOfIngredient) public onlyOwner(msg.sender) {
        // uint256 index = 0;
